...
func NewVaultConfig(vaultConfPath string) (KMSConfig, error) {
	kc := KMSConfig{}
	cfg, err := ini.Load(vaultConfPath)
	if err != nil {
		return kc, err
	}

	config := VaultConfig{
		Endpoint: cfg.Section("vault").Key("endpoint").Value(),
		Auth: VaultAuth{
			Type: "approle",
			AppRole: VaultAppRole{
				ID:     cfg.Section("vault").Key("id").Value(),
				Secret: cfg.Section("vault").Key("secret").Value(),
			},
		},
		Key: VaultKey{
			Version: cfg.Section("vault").Key("version").MustInt(0),
			Name:    cfg.Section("vault").Key("keyname").Value(),
		},
	}
